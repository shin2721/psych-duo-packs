...
レッスン内容が変更に含まれる場合、次の2点を必ず個別に見ること:
6. カード間の整合。締めのカードの各主張が、同じレッスンの種明かしと矛盾していないか。
   種明かしで「例外がある」「判定が割れた」と書いたものを、締めの要約が潰していないか。
   カード単位の検査では捕まらないので、必ず通しで読んで判定する。
7. 締めの主張が curated_sources.json の allowed_claims の範囲に収まっているか。
   締めで新しい推論を作っていないか（誇張は締め・タイトル・プレビューから入る）。
8. 出題した説と、その研究が実際に検証した形が一致しているか。
   通念に複数の意味が混ざっていて（瞬間に効くのか／続けると効くのか等）、
   研究が測っていない側を出題していないか。ズレていれば設問ごと作り直す指摘を出す。
9. 読者が「この研究おかしくない？」とつまずく箇所はどこか。
   その疑問が、ただし書きの後始末ではなく本文の中で先に解消されているか。
10. 本文で意味を変えた言葉が、カードの外に古い意味のまま残っていないか。
   完了画面の一手・持ち帰り行・プレビュー文・選択肢まで含めて読むこと。
   3か所が同じ文字列で揃っていても、全部古い場合がある。
