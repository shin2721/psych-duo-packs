-- Fix: claim_league_reward inserts into user_badges.unlocked_at (not earned_at)
-- Guarded so environments that skipped legacy non-timestamp migrations do not fail.
DO $migration$
BEGIN
  IF to_regclass('public.pending_rewards') IS NULL
    OR to_regclass('public.profiles') IS NULL
    OR to_regclass('public.user_badges') IS NULL THEN
    RAISE NOTICE 'Skipping claim_league_reward replacement: required tables are missing.';
    RETURN;
  END IF;
  ...
  EXECUTE $fn$
    ...
    DECLARE
      ...
      v_new_gems INTEGER;
    BEGIN
      -- Resolve reward row only if it belongs to caller and is still unclaimed.
