...
describe("shop screen architecture", () => {
  test("shop screen delegates major render sections to components", () => {
    ...
    expect(source).toContain('import { isExternalCheckoutBlockedForCurrentPlatform }');
    expect(source).toContain("<ShopSubscriptionsSection");
    expect(source).toContain("<ShopItemsSection");
    expect(source).toContain("subscriptionCheckoutBlockedMessage={subscriptionCheckoutBlockedMessage}");
    ...
    expect(subscriptions).toContain("subscriptionCheckoutBlockedMessage");
    expect(subscriptionCards).toContain("testID={`shop-subscribe-${plan.id}`}");
    expect(subscriptionCards).toContain('testID="shop-subscription-checkout-blocked"');
