...
export function getEnergy() {
  const st = regen(load()); save(st); return st.energy;
}

export function canStart(need = 1) {
...
// レッスン終了時に呼ぶ: 全問正解なら+5
export function onLessonEnd({ asked, correct } = {}) {
  const st = regen(load());
...
export function addEnergy(n) {
  ...
  st.ts = now(); save(st); return st.energy;
}

export function setEnergy(n) {
  const st = load();
