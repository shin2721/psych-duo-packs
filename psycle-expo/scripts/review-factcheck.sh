...
check_contains "app/lesson.tsx" "consumeEnergy," "Energy gate passed into lesson runtime"
check_contains "lib/lesson/useLessonFlow.ts" "params.consumeEnergy(params.lessonEnergyCost)" "Energy gate wired at lesson start"
check_contains "lib/lesson/useLessonLoader.ts" "Analytics.track(\"energy_blocked\"" "Energy blocked analytics wired"
check_contains "lib/lesson/lessonSession.ts" "params.reviewQueueLength > 0 && !params.isReviewRound" "Lesson review round wiring present"
...
check_contains "lib/app-state/types.ts" "startMistakesHubSession: () => {" "MistakesHub start API exposed"
check_contains "docs/REVIEW_FACTCHECK_TEMPLATE.md" "Findings Table (Mandatory)" "Review template exists"
check_contains "docs/REVIEW_FACTCHECK_GUARD.md" "Review Fact-Check Guard" "Reviewer guard instructions present"
