...
const MENTAL_LESSON_METADATA: Record<string, LessonMetadata> = {
  mental_l01: lessonMetadata({
    ...
    lesson_job: "身体の警報と、外でまだ起きていない判決を分ける",
    target_shift: "身体反応の強さを外の失敗や成功の証拠にする状態から、現在の身体報告と未確定の結果を分ける",
    done_condition: "初見の低リスク場面で、身体の報告と外の判決を区別できる",
    takeaway_action: "身体の警報から外の結末を決めそうになったら「警報は鳴っている。判決はまだ。」と1回だけ分ける",
    insight_layer: {
      surprising_question: "身体の警報が強いほど、外で悪い結果が起きることまで確定する？",
      research_finding: "身体感覚は実際の反応だが、その強さだけで外の結果が決まるとは限らない",
      critical_caveat: "この二分法の効果は未証明。具体的な危険や医療上の指示は保留しない",
      usable_scope: "カラオケ、ボウリング、初対面の集まりなど、短い低リスク場面で使う",
      practice_prompt: "身体でいま確認できる報告 / 外でまだ起きていない判決、に分ける",
    },
    load_score: loadScore(2, 1, 1),
    non_goals: [
      "焦りの3タイプ診断",
      "事実・予言・答え合わせへ分けること",
      "呼吸やリアプレイザルなど介入方法を教えること",
      "反芻を止めること",
      "身体症状や具体的な危険を軽視すること",
    ],
  }),
  mental_l02: lessonMetadata({
    ...
    locale_scope: ["ja"],
    sequence_policy: "authored",
    lesson_job: "曖昧な不安の予言を、あとで採点できる形へ変える",
    target_shift: "最悪ケースを事実扱いする状態から、事実・予言・答え合わせに分ける",
    done_condition: "初見の低リスク場面で、事実・予言・答え合わせ時点を1つずつ選べる",
    takeaway_action: "不安が事実に見えたら「事実は？ 予言は？ いつ答え合わせ？」と10秒だけ分ける",
    insight_layer: {
      surprising_question: "不安の予言は、外れても採点基準を変えればずっと当たり続ける？",
      research_finding: "同じ出来事でも、頭が足した意味によって反応が変わることがある",
      critical_caveat: "3行の効果は未証明で、具体的な危険・既知の締切・専門家からの指示は待たない",
      usable_scope: "上司の短文や返信待ちなど、低リスクで情報が足りない日常場面に使う",
      practice_prompt: "事実 / 予言 / 答え合わせ時点、に1つずつ分ける",
    },
    load_score: loadScore(2, 1, 1),
    non_goals: [
      "不安をゼロにすること",
  ...
  mental_l03: lessonMetadata({
    ...
    lane: "core",
    lesson_job: "L1で身体の警報と外の判決を分けた後、次の短い行動を1つ選ぶ",
    target_shift: "見分けるだけで止まる状態から、場面に合う短い戻り方を選べる状態へ移る",
    done_condition: "L1の見分けを前提に、ラベリング・捉え直し・呼吸のどれかを1つ試せる",
    takeaway_action: "焦ったら10秒だけ、身体ラベルか長い息を1回使う",
    insight_layer: {
      surprising_question: "焦りは消そうとするほど扱いづらくなる？",
      research_finding: "感情を短く名付けるだけでも、反応との距離を作れる可能性がある",
      critical_caveat: "強い症状を10秒介入だけで解決できるという意味ではない",
      usable_scope: "会議前、通知直後、急ぎの連絡など軽い焦りの立ち上がりに使える",
      practice_prompt: "身体ラベル / 捉え直し / 長い息、から1つ選ぶ",
    },
