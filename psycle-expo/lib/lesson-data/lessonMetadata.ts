...
const MENTAL_LESSON_METADATA: Record<string, LessonMetadata> = {
  ...
  mental_l02: lessonMetadata({
    ...
    locale_scope: ["ja"],
    sequence_policy: "authored",
    preview_prompt: "その一文に、実際は何と書いてあった？",
    lesson_job: "画面に残っている文字と頭が足した意味を分け、予言を採点できる形へ戻す",
    target_shift: "頭が足した意味を文面そのものとして扱う状態から、両者を分けて見る",
    done_condition: "初見の低リスク場面で、確認可能な事実・反証可能な予言・向こうから届く答え合わせを選べる",
    takeaway_action: "今日、不安が事実の顔で届いたら、一度だけ聞く。「画面に残っているのは、どこまで？」",
    insight_layer: {
      surprising_question: "不安が書き換えるのは、届いた文面ではなく、こちらの控えの方？",
