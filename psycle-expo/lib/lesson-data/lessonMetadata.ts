...
const MENTAL_LESSON_METADATA: Record<string, LessonMetadata> = {
  ...
  mental_l02: lessonMetadata({
    ...
    locale_scope: ["ja"],
    sequence_policy: "authored",
    preview_prompt: "スクリーン1時間で、睡眠は何分減る？",
    lesson_job: "通念の効果量を実測と突き合わせ、守るべきルールを1つに絞る",
    target_shift: "時間で引いたルール(就寝◯時間前から禁止)から、場所で引いたルール(ベッドに持ち込まない)へ",
    done_condition: "捨てていいルールと残すルールを、効果量を根拠に言い分けられる",
    takeaway_action: "今夜から、スマホをベッドに持ち込まない。とくにベッドの中で返信とゲームをしない。",
    insight_layer: {
      surprising_question: "「就寝2時間前からスクリーン禁止」は、測ると何分ぶんの効果がある？",
      research_finding: "スクリーン1時間あたりの睡眠短縮は3〜5分。就寝直前に絞ると効果は消え、ベッドに入ってからだけ効果が残る",
      critical_caveat: "すべて横断研究で因果は未証明。対象の多くは子どもと若者。「寝る前のスマホは無害」ではない",
      usable_scope: "広く信じられている睡眠衛生ルールのうち、どれを守るか決めるときに使う",
      practice_prompt: "総量 → 就寝直前 → ベッドの中、と絞って効果を見る",
    },
    load_score: loadScore(1, 1, 1),
    non_goals: [
      "寝る前のスマホは無害だと言うこと",
  ...
  mental_l07: lessonMetadata({
    lesson_id: "mental_l07",
    lane: "core",
    locale_scope: ["ja"],
    sequence_policy: "authored",
    preview_prompt: "発散すると、怒りは本当に減る？",
    ...
    takeaway_action: "「発散しなきゃ」をやめる。怒り対策として走るのも、いったん手放していい。",
    insight_layer: {
    ...
    non_goals: [
      "発散が逆効果だと言うこと",
      "その場で怒りを鎮める方法として提示すること",
      "運動の健康上の利益を否定すること",
      "暴力や生活の破綻につながる怒りへの支援を遅らせること",
    ],
  }),
