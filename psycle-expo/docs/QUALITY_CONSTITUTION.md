...
## 執筆チェックリスト（エピソード型カード：Claude / Codex 共通）

読み物として書いた回（Discovery系エピソード）は、書き終えたらこの11項目を通す。
文字数の上限は予算ではなく壁アラームとして置いてある。本質が入らないから削る、が起きたら
上限のほうを疑う。壁の防止は字数ではなく構造（1画面1ビート・答えはボックスだけが言う・
ただしは分離）で持つ。
機械で検査できるものは `scripts/validate-lessons.ts` が自動で落とすので、ここには
機械が見られない判断だけを置く。違反したらそのカードは reject。

0. **一次資料の全文を読んでから書く。要約から書き始めない。**
   台帳（curated_sources.json）には効果量だけでなく、**何を・誰で・どう測ったか**を必ず記録する
   ——測定の種類と尺度（例：いま感じている怒り／怒りっぽい性格）、対象（挑発したか）、
   デザイン（回数・期間）、各数字の根拠件数。**論文に書かれていない項目は「未報告」と明記する。**
   空欄のまま書くと、書き手は推測で埋めて、埋めたことに自分で気づかない。
   画面の各文は台帳の行と突き合わせる。根拠行のない文は書かない。

0.1 **出題する説が、その研究が実際に検証した形かを、書く前に判定する。**
   通念には複数の意味が混ざっていることが多い（「発散するといい」＝カッとなった瞬間に
   出すとスッキリする／ふだんから出しておくと軽くなる）。研究が測ったのはどちらか、
   台帳の測定・対象の記録で確かめてから出題を決める。**ズレたまま書くと、どれだけ
   文を直しても種明かしが設問と噛み合わず、直すたびに別の場所が破綻する。**
   研究が答えられない形の問いは、出題にしない——答えられる形に問いを替えるか、
   その弾を捨てる。研究が答えられないことは、ただし書きで正直に「まだ分かっていない」
   と言う。
   方法を描写する語（やった／続けた／ふだん／瞬間）も台帳の記述からしか選ばない。
   台帳が区別していない条件差を、動詞や副詞で作らない。
   対比を書くときは「同じ比べ方で」と頭に置けるかを確かめ、置けないなら対比自体を書かない。

1. **1文目はアプリが本当に言える文にする。**
   通念を引用する／誰が確かめたかを言う／問う。読者の一日を情景として代筆しない。
   語り手の伝記（「私もそう思っていました」）を作らない。素材が強ければ飾りは要らない。

2. **断定の強さは証拠の強さに連動させる。判定は比喩で言わない。**
   大規模メタ分析の頑健な結果は言い切る。信頼区間がゼロに近い、研究数が少ない、
   自己申告依存といった弱さがあるものは、報告と同じ息で留保を付ける。
   判定の言い方は4つに固定する——**効いた／効かなかった／白黒つかない／
   調べた研究が足りなくて分からない**。「効果がぼやけた」「宙に浮いている」
   のような絵は、同じ絵を持っている読者にしか通じず、しかも「効かなかった」と
   誤読される。比喩を使ってよいのは構造の説明だけ（「消火器の使い方ではなく
   防火訓練」＝緊急時に使うのか平時に鍛えるのか）で、判定そのものには使わない。
   効果の大きさも同様に、統計の物差しを言い換えた造語（「中くらいの差」）ではなく、
   読者の物差しで渡す——自分との比（3分の1に縮んだ）か、同じ研究内で**読者がすでに
   読んだ**効果との比較（約2倍）。まだ明かしていないカードの効果を物差しに使わない。
   画面で正直に渡せる物差しがなければ、方向と確かさ（誤差では説明のつかない差）で
   止め、大きさは詳細シートに置く。シートでは凡例つきの研究の目安
   （0.2=小、0.5=中、0.8=大）で数字を示してよい。凡例のない「中くらい」を画面に出さない。
   詳細シートの「確からしさ」の行は、結論の1語で始めて理由を続ける——
   **かなり固い／まあ固い／まだ揺れる／薄い**。語は等級ではなく信じ方で、理由は台帳の行
   （研究数、誤差の幅、事前登録、偏りの検査、無作為割り付け、測り方）からしか書かない。
   「限界」との線引き: 直っても判定の固さが変わらないもの（測っていないもの、入っていない人、
   効果の持続）は限界、直れば固さが変わるもの（数、精度、偏り、測り方）は確からしさ。
   同じ事実を両方の欄に書かない。

3. **ただし書きは限界の列挙で終わらせず、受け取り方の一文で締める。**
   「〜とまでは言えない。言えるのは〜まで」「〜くらいが妥当な線」。
   指図口調（「〜と受け取ってください」）は使わない。

4. **締めのカードは、台帳の allowed_claims にある主張か、その弱め版しか言えない。**
   締めで新しい推論を作らない。作りたくなったら先に台帳へ登録する。
   誇張は圧縮点（締め・タイトル・プレビュー）から入る。ここが一番危ない。

5. **締めは、同じレッスンの種明かしと矛盾してはならない。**
   種明かしで「例外がある」と書いたものを、締めの要約で潰さない。
   カード単位の検査では捕まらないので、必ず別のエージェントが通しで読む。

6. **出口は義務を減らす。** 新しい practice を宿題として足さない。
   捨てていいものと残るものを並べ、読者の判断材料にする。

7. **選択肢は、例まで書いて初めて選択肢になる。主語も省かない。**
   それだけ読んで何の話か分かる文にする（「はっきり減った」ではなく
   「続けた人たちの方が、はっきり怒りっぽくなかった」）。
   「考え方に触れる要素を含むもの」ではなく「考え方の練習も入れる（浮かんだ考えを、
   事実ではなく考えとして眺める）」。抽象名だけでは、何を選んだのか分からないまま
   正解が決まる。あわせて、最長・最も慎重・技能記述の復唱が正解になっていないか見る。
   形式で正解が当てられる設問は、賭けとして成立していない。

8. **各画面は、それだけ読んで意味が立つ。**
   「この1万人」のような指示語、省略した主語・目的語、画面外に置いてきた前提が
   残っていたら書き直す。設問だけでなく、種明かしとただし書きも同じ。
   確認は必ず、他のカードを見ずに1画面だけを読んで行う。書き終えた直後の書き手は
   全カードと論文を頭に入れているので、そのまま読み返しても省略に気づけない。

   **通し読みの対象は、カード5枚ではなく、読者が目にする全文字列。**
   完了画面の一手（metadata の takeaway_action）、カードの持ち帰り行
   （actionable_advice）、プレビュー文、選択肢——カードの外にも読者向けの文章がある。
   本文で言葉の意味を変えたら、その言葉を使っている画面外の文字列を全部洗い直す。
   実例：カード2で「発散」に球技・体育まで含むと認めたのに、持ち帰りは
   「発散しなきゃをやめる」のまま残り、球技まで捨てる話に読めていた。
   3か所とも同じ文字列で揃っていたので、機械の同期検査では捕まらない。
   揃ったまま全部古い、という壊れ方は、通し読みでしか見つからない。

   **読者がつまずく疑問は、あとから注釈で塞がず、本文がその場で答える。**
   「この研究おかしくない？」と思われた時点で、その手前の文が仕事をしていない。
   たとえば測ったものを日常の言葉で先に言えば（「イライラや怒りの度合いを比べる」）、
   「怒っていない人で試して意味あるの？」という疑問はそもそも生まれない。
   ただし書きは、生まれてしまった疑問の後始末ではなく、正直な限界のための場所。

9. **弾の4基準を満たすものだけをエピソードにする。**
   読者が既に持っている迷いか／通念の反転があるか／裁定で意思決定が軽くなるか／
   証拠の等級が言い切りに耐えるか。確認するだけの知見はエピソードにしない。

10. **専門用語も内輪話も、読者の言葉に変えるか、使わない。**
    「コクラン」→「医療エビデンス評価の総本山・コクラン」。
    調べて面白かったことでも（「著者自身の理論が自分のデータで破れている」など）、
    その画面単体で意味が立たない一文は削る。書き手が調べた面白さと、読者に届く
    面白さは別物。ただし書きも同じで、3つを略記で詰め込むより1つを平たく言う。
    枠に収める方法は「全部を縮める」ではなく「言うことを減らす」。残りは詳細側にある。
