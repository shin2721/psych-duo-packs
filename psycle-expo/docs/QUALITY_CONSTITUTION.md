...
7. **選択肢には例を入れる。** 「考え方に触れる要素を含むもの」ではなく
   「考え方の練習も入れる（浮かんだ考えを、事実ではなく考えとして眺める）」。
   抽象名だけの選択肢は、何を選んだのか分からないまま正解が決まる。

8. **正解肢を形式で検出可能にしない。**
   最長・最も慎重・技能記述の復唱が正解になっていたら書き直す。

9. **弾の4基準を満たすものだけをエピソードにする。**
   読者が既に持っている迷いか／通念の反転があるか／裁定で意思決定が軽くなるか／
   証拠の等級が言い切りに耐えるか。確認するだけの知見はエピソードにしない。

10. **専門用語は一言で武器に変えるか、使わない。**
    「コクラン」→「医療エビデンス評価の総本山・コクラン」。
    知らない固有名詞をそのまま置くと、権威が伝わらず読者が置き去りになる。
